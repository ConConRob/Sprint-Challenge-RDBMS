...
RDMS is database management system, it is the database software. SQL is a language that is typicaly used to query the RDBMS

1. Why do tables need a `primary key`?

Tables have a primary key so they can be refrenced

1. What is the name given to a table column that references the primary key on another table.

This key is called a foreign key

1. What do we need in order to have a _many to many_ relationship between two tables.

We need to have a third table that references both other tables primary key
